...
PCIDevice::PCIDevice(int bus_num, int device_num)
{
...
void PCIDevice::writeRegister(int register_offset, long data)
{
...
int PCIDevice::readRegister(int register_offset)
{
    unsigned long address = 0x80000000 | (bus_num << 16) | (device_num << 11) | (register_offset & 0xFC);
    outl(PCI_CONFIG_ADDRESS, address);
